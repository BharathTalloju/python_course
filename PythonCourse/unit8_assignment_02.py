...
def get_vowel_pos(word):
    """Returns the first position on vowel encountered in the word,
...
def pig_latin(word, vowel_pos):
    """Returns the word in pig-latin form"""
    ...
    word = (word[vowel_pos:] + word[:vowel_pos] + 'ay').lower()
...
while(True):
    ...
    words = [pig_latin(word, get_vowel_pos(word)) for word in words]
    sentence = " ".join(words)
    print sentence
