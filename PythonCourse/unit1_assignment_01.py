...
def get_multiples_desc(number, count):
    ...
    """
    return range(count*number, number - 1, -number)
...
def ascii_distance(first, second):
    ...
    """
    return ord(max(first,second)) - ord(min(first,second)) # use 2 builtin functions to get your work done.
...
def count_binary_ones(number):
    ...
    return (list(bin(number)[2:])).count('1')
