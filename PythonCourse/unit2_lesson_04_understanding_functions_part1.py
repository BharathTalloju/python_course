...
def map_no_of_vowels_to_respective_strings(input):
    return dict(zip(map(count_no_of_vowels,input), input))
...
def get_word_with_least_vowels(input):
    ...
    """
    d = map_no_of_vowels_to_respective_strings(input)
    return d[min(d)] # replace this with right code. Use min builtin and define a new function to pass as key.
...
def get_min_max_words(input):
    ...
    """
    d = dict(zip(map(len,input),input));
    return d[min(d)],d[max(d)] # replace this calls to min and max
...
-   Functions are objects and can also be passed as arguments
-   functions can returrn multiple values as tuples
-   Python functions use call-by-sharing model
"""
