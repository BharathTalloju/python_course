...
def left_binary_search(input, value):
    if input.__class__ is not list:
        raise TypeError,"Only lists are accepted as argument1"

    l = len(input)
    if l == 0:
        return -1

    if (value < input[0]) or (value > input[l-1]):
        return -1
    end_pos = l - 1
    mid = end_pos/2
    if input[mid] == value:
        while input[mid-1] == value:
            mid -= 1
        return mid
    if input[mid] < value:
        return mid+left_binary_search(input[mid+1:], value)
    if input[mid] > value:
        return left_binary_search(input[:mid], value)


# write your own exhaustive tests :)
def test_left_binary_search_student():
    assert left_binary_search([1,2,3,3,4,4,5], 3)
    pass
